...
export class CartPage implements OnInit {
        ...
        (data: any) => {
          ...
          if (this.config.appNavigationTabs)
            this.navCtrl.navigateForward(
              this.config.currentRoute + "/product-detail/" + data.id
            );
          else this.navCtrl.navigateForward("/product-detail/" + data.id);
        },
        (err) => {
          this.loading.hide();
          console.log(err);
  ...
  async proceedToCheckOut() {
    if (
      this.shared.customerData.id == null ||
      ...
      if (this.config.checkOutPage == 1) this.shared.onePageCheckOut();
        ...
        if (this.config.appNavigationTabs)
          this.navCtrl.navigateForward(
            this.config.currentRoute + "/billing-address"
          );
        else this.navCtrl.navigateForward("/billing-address");
      }
    }
  }
  //============================================================================================
  openProductsPage() {
    ...
    else this.navCtrl.navigateForward(this.config.getCurrentHomePage());
  }
  //============================================================================================
  ionViewWillEnter() {
  ...
  changingCart() {
    ...
    this.shared.couponArray.forEach((value) => {
      this.products = this.couponProvider.apply(
        value,
  ...
  getCoupon = function (code) {
    ...
    this.config.getWoo("coupons?code=" + code).then(
      (data: any) => {
        this.loading.hide();
        let d = data;
        let coupon = d[0];
        if (d.length == 0) this.shared.showAlert("Invalid Coupon Code!");
        else this.applyCouponCart(coupon);
      },
      (error) => {
        this.loading.hide();
        console.log(error);
  ...
  applyCouponCart = function (coupon) {
    ...
    if (
      this.couponProvider.validateCouponService(
        coupon,
        this.products,
